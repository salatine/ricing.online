...
def main():
    ...
    args.add_argument("--ignore-mtime",
                      action="store_true",
                      help="If set, will set mtime as 0 for all files. Useful for reproducible builds.")
    ...
    if tar:
        (root, total_size) = handle_tar(logger, tar, args.ignore_mtime)
    else:
        (root, total_size) = handle_dir(logger, path, args.exclude, args.ignore_mtime)
...
def handle_dir(logger, path, exclude, ignore_mtime):
    path = path + "/"
    exclude = exclude or []
    ...
    def make_node(st, name):
        ...
        obj[IDX_MTIME] = int(st.st_mtime) if not ignore_mtime else 0
...
def handle_tar(logger, tar, ignore_mtime):
    mainroot = []
    filename_to_hash = {}
    total_size = 0
    ...
    for member in tar.getmembers():
        ...
        obj[IDX_MTIME] = member.mtime if not ignore_mtime else 0
